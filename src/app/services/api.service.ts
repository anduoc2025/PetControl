import { Injectable } from '@angular/core';
import { HttpClient } from '@angular/common/http';
import { Observable } from 'rxjs';
import { map } from 'rxjs/operators';

@Injectable({
  providedIn: 'root'
})
export class ApiService {

  private apiUrl = 'https://dog.ceo/api';

  constructor(private http: HttpClient) { }

  /**
   * Obtiene la lista completa de razas de perros.
   */
  getBreeds(): Observable<string[]> {
    return this.http.get<any>(`${this.apiUrl}/breeds/list/all`).pipe(
      map(response => Object.keys(response.message))
    );
  }

  /**
   * Obtiene una imagen aleatoria para una raza específica.
   * @param breed - La raza del perro.
   */
  getRandomImageByBreed(breed: string): Observable<string> {
    return this.http.get<any>(`${this.apiUrl}/breed/${breed}/images/random`).pipe(
      map(response => response.message)
    );
  }
}
